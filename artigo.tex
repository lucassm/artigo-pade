...
\begin{description}[\IEEEsetlabelwidth{TCP/IP}\IEEEusemathlabelsep]
...
% Na \autoref{tab:desafios} são apresentados os principais desafios identificados pelos autores em [38], [39] para aplicação do MAS na automação de sistemas elétricos.
...
% Já o conceito de programação orientada a eventos (assíncrono), refere-se à característica do fluxo do programa ser determinado por eventos externos. Esse tipo de paradigma utiliza sempre um loop de eventos chamado (reactor loop) assim como métodos que são chamados quando um evento acontece (callbacks).  Na \autoref{fig:reactor} é mostrada uma representação do loop de eventos Twisted e da chamada de uma callback quando um evento é processado.
...
\begin{figure}[htb]
  \centering
  \lstinputlisting[language=Python,showstringspaces=false,stepnumber=1,breaklines=true,lastline=50,basicstyle=\footnotesize]{agente_teste_1.py}
